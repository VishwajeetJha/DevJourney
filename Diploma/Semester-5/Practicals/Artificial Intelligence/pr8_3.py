class TicTacToe:
    def __init__(self):
        ...
        self.current_player = None  # Will be set during symbol selection
        self.player_symbols = ['X', 'O']  # Default symbols

    def select_symbols(self):
        while True:
            player_x = input("Player 1, choose your symbol (X/O): ").upper()
            if player_x in self.player_symbols:
                player_o = 'O' if player_x == 'X' else 'X'
                self.current_player = player_x
                print(f"Player 1 is {player_x}, Player 2 is {player_o}.")
    ...
    def make_move(self, position):
        if 0 <= position < 9:  # Ensure position is within valid range
    ...
    def play(self):
        player_x, player_o = self.select_symbols()
        self.current_player = player_x  # Set starting player
